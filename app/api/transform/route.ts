...
- 일본어, 중국어 문자 사용 금지
- 러시아어/키릴문자, 베트남어, 그 외 한국어·영어 고유명사가 아닌 모든 외국 문자 절대 금지. 외국어 개념은 한국어로 번역해 쓴다.
...
JSON 형식으로만 응답해주세요.`;
    ...
    // 핵심역량 추출기는 영어 기술명(React·SQL 등)을 정당하게 쓰므로 allowLatin:true.
    // (러시아어·베트남어·CJK 등 비영어 외국 문자만 차단)
    const result = await generateGuardedKorean<{
      matchScore: number;
      competencies: Array<{ title?: string; description?: string; source?: string }>;
      gaps: Array<{ requirement?: string; suggestion?: string }>;
    }>({
      system: SYSTEM_PROMPT,
      user: userMessage,
      isValid: (o) => {
        const p = o as { matchScore?: unknown; competencies?: unknown; gaps?: unknown };
        return typeof p.matchScore === "number" && Array.isArray(p.competencies) && Array.isArray(p.gaps);
      },
      guardTexts: (o) => {
        const t: string[] = [];
        for (const c of o.competencies ?? []) t.push(c.title ?? "", c.description ?? "", c.source ?? "");
        for (const g of o.gaps ?? []) t.push(g.requirement ?? "", g.suggestion ?? "");
