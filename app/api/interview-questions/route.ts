...
- 일본어, 중국어 문자 사용 금지
- 러시아어/키릴문자, 베트남어 등 한국어·영어 고유명사가 아닌 모든 외국 문자 절대 금지. 외국어 개념은 한국어로 번역해 쓴다.
...
반드시 JSON 형식으로만 응답해주세요.`;
    ...
    // 면접 질문은 영어 기술명(React·SQL 등)을 정당하게 쓰므로 allowLatin:true.
    // (러시아어·베트남어·CJK 등 비영어 외국 문자만 차단)
    type IqQuestion = { question?: string; intent?: string; hint?: string; frequency?: string };
    type Iq = {
      summary?: { totalQuestions?: number; focusCategories?: string[]; resumeGaps?: string[] };
      questions: IqQuestion[];
    };
    const result = await generateGuardedKorean<Iq>({
      system: SYSTEM_PROMPT,
      user: userMessage,
      models: ["llama-3.1-70b-versatile", "llama-3.3-70b-versatile"],
      isValid: (o) => {
        const p = o as { summary?: unknown; questions?: unknown };
        return !!p.summary && Array.isArray(p.questions) && p.questions.length > 0;
      },
      guardTexts: (o) => {
        const t: string[] = [];
        for (const q of o.questions ?? []) t.push(q.question ?? "", q.intent ?? "", q.hint ?? "");
        for (const f of o.summary?.focusCategories ?? []) t.push(f);
        for (const g of o.summary?.resumeGaps ?? []) t.push(g);
    ...
    // summary 구조 정규화 (필드 누락 방어)
    const s = result.data.summary;
    const normalized = {
      ...result.data,
      summary: {
        totalQuestions: s?.totalQuestions ?? result.data.questions.length,
        focusCategories: Array.isArray(s?.focusCategories) ? s!.focusCategories : [],
        resumeGaps: Array.isArray(s?.resumeGaps) ? s!.resumeGaps : [],
      },
    };
